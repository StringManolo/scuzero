...
class GpsMonitorService : Service() {
  ...
  private val gpsAccessLogs = ConcurrentLinkedQueue<String>()
  ...
  companion object {
    const val NOTIFICATION_ID = 12349
    const val CHANNEL_ID = "gps_monitor_channel"
    const val ALERT_CHANNEL_ID = "gps_alert_channel"
  ...
  private fun createNotificationChannels() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      val monitorChannel = NotificationChannel(
        ...
        description = "Monitors GPS access continuously"
      }
      ...
      val alertChannel = NotificationChannel(
        ...
        description = "Alerts when GPS is accessed"
  ...
  private fun createMonitoringNotification(): Notification {
    ...
    .setContentTitle("GPS Privacy Protection")
    .setContentText("Monitoring GPS access...")
  ...
  private fun createAlertNotification(appName: String, packageName: String): Notification {
    ...
    .setContentTitle("📍 GPS Access Detected")
    .setContentText("$appName is accessing location")
      ...
      while (isMonitoring) {
        try {
          ...
          if (foregroundApp != null && !previousForegroundApps.contains(foregroundApp)) {
            if (!shouldIgnoreApp(foregroundApp) && hasGpsPermission(foregroundApp)) {
              ...
              if (shouldAddLog("FOREGROUND_APP", appName, foregroundApp, currentTime)) {
                ...
                showGpsAlert(appName, foregroundApp)
              }
            }
          }
      ...
      while (events.hasNextEvent()) {
        ...
        when (event.eventType) {
          UsageEvents.Event.ACTIVITY_RESUMED -> {
            if (hasGpsPermission(event.packageName)) {
              ...
              if (shouldAddLog("ACTIVITY_RESUMED", appName, event.packageName, currentTime)) {
                addLogEntry(logEntry)
                showGpsAlert(appName, event.packageName)
              }
            }
          }
  ...
  private fun hasGpsPermission(packageName: String): Boolean {
    return try {
      val packageManager = applicationContext.packageManager
      val fineLocationCheck = packageManager.checkPermission(
        android.Manifest.permission.ACCESS_FINE_LOCATION,
        packageName
      )
      val coarseLocationCheck = packageManager.checkPermission(
        android.Manifest.permission.ACCESS_COARSE_LOCATION,
        packageName
      )
      fineLocationCheck == PackageManager.PERMISSION_GRANTED || coarseLocationCheck == PackageManager.PERMISSION_GRANTED
    } catch (e: Exception) {
      false
    }
  }
  ...
  private fun getAppDetailedInfo(packageName: String): String {
    return try {
      ...
      .requestedPermissions?.filter {
        it.contains("LOCATION") || it.contains("GPS")
      }?.joinToString(", ") ?: "No specific location permissions"

      val locationType = when {
        hasFineLocation(packageName) -> "Fine (GPS) Location"
        hasCoarseLocation(packageName) -> "Coarse (Network) Location"
  ...
  private fun hasFineLocation(packageName: String): Boolean {
    return try {
      val packageManager = applicationContext.packageManager
  ...
  private fun hasCoarseLocation(packageName: String): Boolean {
    return try {
      val packageManager = applicationContext.packageManager
      packageManager.checkPermission(
        android.Manifest.permission.ACCESS_COARSE_LOCATION,
  ...
  private fun addLogEntry(logEntry: String) {
    gpsAccessLogs.add(logEntry)
    latestLogs.add(logEntry)

    if (gpsAccessLogs.size > 50) {
      gpsAccessLogs.poll()
    }
  ...
  private fun saveLogToFile(logEntry: String) {
    try {
      ...
      val logFile = File(directory, "gps_logs.txt")
  ...
  private fun showGpsAlert(appName: String, packageName: String) {
  ...
  private fun showBackgroundToast(appName: String) {
    try {
      Toast.makeText(this, "📍 GPS accessed by: $appName", Toast.LENGTH_LONG).show()
    } catch (e: Exception) {
    }
  }
  ...
  fun getLogs(): List<String> {
    return gpsAccessLogs.toList().reversed()
  }
